...
def take_input():
    ...
    if tallenna.get() == 1:
        a = np.array(sana)
        b = np.array(paikka)
        c = np.array(str(duunitori_luku.get()) + str(tyokkari_luku.get())+ str(option.get()))
        d = np.array(duunitori_tekstista.get())
        data = np.row_stack((a, b, c, d))
        np.savetxt('tallennettu_haku.dat', data, fmt = '%s', encoding='utf-8')
        #np.savetxt('tallennettu_haku.dat', data)
        data1 = np.loadtxt('tallennettu_haku.dat', dtype='str', encoding='utf-8')
        print(data1)
...
#Tallennettu haku
#tk.Button(window, text='Tallennettu haku', command=hakusivun_valinta).grid(row=14,column=1, columnspan=2, padx=5)
