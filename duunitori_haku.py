...
def paiva_muoto(y): #jos päivämäärä sisältää vuoden, ei lisätä vuotta. Jos ei sisällä vuotta, lisätään tämä vuosi.
    if len(y) > 6:
        ...
        y = date(*(time.strptime(y + str(date.today().year), '%d.%m.%Y')[0:3]))
    return y;
...
def yhden_sivun_tulokset(url, y, estot, kielletyt_kategoriat):
    ...
    for tulos in tulokset:
        ...
        if erotus < (paivaraja+1):
            ...
            #print("tultiin kategorian tarkistukseen 1")
            if kategoria in kielletyt_kategoriat:
                continue
            temp_julkaistu.append(koko_julkpaiva)
            temp_titteli.append(tulos.findChild('div', class_ = 'job-box__content').findChild('h3').text)
...
def suorita_haku_duunitori(hakusana, hakusijainti, negaatiot, haku_tekstista, julkpvm):
    #negaatiot on siihen kenttään syötetyt sanat
    kielletytKategoriat = kieltolista_luku()
    temp_titteli, temp_tyonantaja, temp_sijainti, temp_linkki, temp_julkaistu, temp_kategoria, osuvuus = [], [], [], [], [], [], []
    if len(hakusana.split(" ")) > 1:
        hakusana = hakusana.replace(" ", "%3B")
    if len(hakusijainti.split(" "))> 1:
        hakusijainti = hakusijainti.replace(" ", "%3B")
        ...
        for s in hakusana.split(" "):
            for p in hakusijainti.split(" "):
                for n in range(1, sivumaara + 1):
                    sivu_titteli, sivu_tyonantaja, sivu_sijainti, sivu_linkki, sivu_julkaistu, sivu_kategoria = yhden_sivun_tulokset(("https://duunitori.fi/tyopaikat/?haku=" + hakusana + "&alue=" + hakusijainti + haku_tekstista + "&order_by=date_posted" + "&sivu=" + str(n)), julkpvm, negaatiot, kielletytKategoriat)
